...
void CAN_Config(CAN_Num CAN,unsigned short int  BaudRate)
{
	...
	//开始配置CAN总线波特率
	ECan_Reg_Shadow.CANMC.all     = ECanReg->CANMC.all;
	ECan_Reg_Shadow.CANMC.bit.CCR = 1;
	ECanReg->CANMC.all            = ECan_Reg_Shadow.CANMC.all;
	...
	while (ECan_Reg_Shadow.CANES.bit.CCE != 1);
	...
	while (ECan_Reg_Shadow.CANES.bit.CCE != 0);
...
{
	EALLOW;
	ECanaRegs.CANMIM.bit.MIM31 = 0;//使能中断邮箱31的中断;
	ECanaRegs.CANMIL.bit.MIL31 = 0;//将中断31连接至中断1;
	ECanaRegs.CANGIM.bit.I1EN = 0;//使能中断1;
	EDIS;
}
